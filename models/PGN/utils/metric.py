...
def cal_mul_sums_n(refs, preds, n=1, mode='f'):
    total_score = 0.
    for ref, pred in zip(refs, preds):
        total_score += compute_rouge_n(pred, ref, n=n, mode=mode)
    return total_score / len(refs)

def cal_mul_sums_l(refs, preds, mode='f'):
    total_score = 0.
    for ref, pred in zip(refs, preds):
        total_score += compute_rouge_l(pred, ref, mode=mode)
    return total_score / len(refs)
