...
def build_batchers(net_type, word2id, cuda, debug, augment):
    train_name = 'train_augmented' if augment else 'train'
    ...
    train_loader = DataLoader(
        ExtractDataset(train_name), batch_size=BUCKET_SIZE,
        shuffle=not debug,
...
def main(args):
    ...
    train_batcher, val_batcher = build_batchers(args.net_type, word2id,
                                                args.cuda, args.debug, args.augment)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--ckpt_freq', type=int, action='store', default=250,
