...
def decode(save_path, model_dir, split, batch_size,
    ...
    with torch.no_grad():
        for i_debug, raw_article_batch in enumerate(loader):
            ...
            for j, n in ext_inds:
                ...
                preds.append(' '.join(decoded_sents))
                i += 1
                print('{}/{} ({:.2f}%) decoded in {} seconds\r'.format(
                    i, n_data, i/n_data*100,
                    timedelta(seconds=int(time()-start))
                ), end='')
    with open(join(save_path, 'output/preds.txt'), 'w') as f:
        for pred in preds:
            f.write(pred + '\n')
    # read refs
    refs = []
    with open('dataset/refs.txt', 'r') as f:
    ...
    with open('dataset/ref_ids.txt', 'w') as f:
        for ref in ref_ids:
            f.write(ref + '\n')
    with open('dataset/pred_ids.txt', 'w') as f:
        for pred in pred_ids:
            f.write(pred + '\n')
    files2rouge.run('dataset/pred_ids.txt', 'dataset/ref_ids.txt')
    print()
